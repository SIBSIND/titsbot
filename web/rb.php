...
class RDefault implements Logger
{
     ...
     *
     * @param int $mode mode of operation for logging object
     *
     * @return self
     */
    ...
    public function grep($needle)
    {
        ...
        foreach ($this->logs as $logEntry) {
            if (mb_strpos($logEntry, $needle) !== false) {
                $found[] = $logEntry;
            }
        }
        return $found;
...
class Debug extends RDefault implements Logger
{
    ...
    protected function fillInValue($value)
    {
        ...
        $value = strval($value);
        if (mb_strlen($value) > ($this->strLen)) {
            $value = mb_substr($value, 0, ($this->strLen)).'... ';
        }
    ...
    protected function output($str)
    {
        ...
        if (!$this->mode) {
            ...
            if (mb_strpos($str, 'CREATE') === 0
            || mb_strpos($str, 'ALTER') === 0
            || mb_strpos($str, 'DROP') === 0) {
                $highlight = true;
            }
            if (PHP_SAPI === 'cli') {
                if ($highlight) {
    ...
    protected function normalizeSlots($sql)
    {
        $newSql = $sql;
        $i = 0;
        while (mb_strpos($newSql, '?') !== false) {
            $pos = mb_strpos($newSql, '?');
            $slot = ':slot'.$i;
            $begin = mb_substr($newSql, 0, $pos);
            $end = mb_substr($newSql, $pos + 1);
            if (PHP_SAPI === 'cli') {
...
interface Driver
{
     ...
     * @return mixed
     */
    public function GetCursor($sql, $bindings = []);
     ...
     *
     * @param bool   $tf           TRUE = debug mode ON
...
class RPDO implements Driver
{
    ...
    public function __construct($dsn, $user = null, $pass = null)
    {
        ...
        if ((mb_strpos($this->dsn, 'sqlite') === 0 && PHP_MAJOR_VERSION === 5 && PHP_MINOR_VERSION === 3) || defined('HHVM_VERSION') || $this->dsn === 'test-sqlite-53') {
            $this->max = 2147483647; //otherwise you get -2147483648 ?! demonstrated in build #603 on Travis.
        } elseif (mb_strpos($this->dsn, 'cubrid') === 0) {
     ...
     *
     * @param bool $yesNo pass TRUE to bind all parameters as strings.
     *
     * @return void
     */
     ...
     *
     * @param int $max maximum value for integer bindings
     *
     * @return int
     */
    ...
    public function GetCursor($sql, $bindings = [])
    {
        $statement = $this->runQuery($sql, $bindings, [ 'noFetch' => true ]);
     ...
     * results.
     *
     * @param bool   $trueFalse turn on/off
     ...
     *
     * @param bool $enable TRUE to enable logging
     *
     * @return self
     */
...
class OODBBean implements\IteratorAggregate, \ArrayAccess, \Countable, Jsonable
{
     ...
     *
     * @param int           $mode error handling mode
    ...
    public static function setErrorHandlingFUSE($mode, $func = null)
    {
        ...
        $old = [ self::$errorHandlingFUSE, self::$errorHandler ];
     ...
     *
     * @param bool $automatic TRUE to enable automatic resolving aliased parents
     *
     * @return void
     */
    public static function setAutoResolve($automatic = true)
    {
        self::$autoResolve = (bool) $automatic;
    }
    ...
    private function parseJoin($type)
    {
        $joinSql = '';
        $joins = [];
        if (mb_strpos($this->withSql, '@joined.') !== false) {
    ...
    private function getOwnList($type, $redbean)
    {
        ...
        if ($this->getID()) {
            ...
            if (!is_numeric($firstKey) || $firstKey === null) {
                ...
                $beans = $redbean->find($type, [], " {$joinSql} $myFieldLink = :slot0 ".$this->withSql, $bindings);
            } else {
                $bindings = array_merge([ $this->getID() ], $this->withParams);

                $beans = $redbean->find($type, [], " {$joinSql} $myFieldLink = ? ".$this->withSql, $bindings);
            }
        }
     ...
     * @param bool         $notrim    if TRUE selection keys will NOT be trimmed
     *
     * @return OODBBean
     */
     ...
     * the resulting array.
     *
     * @param bool  $meta    set to TRUE if you want to export meta data as well
     * @param bool  $parents set to TRUE if you want to export parents as well
     * @param bool  $onlyMe  set to TRUE if you want to export only this bean
    ...
    public function export($meta = false, $parents = false, $onlyMe = false, $filters = [])
    {
        ...
        if ($parents) {
            foreach ($this as $key => $value) {
                if (mb_substr($key, -3) !== '_id') {
                    continue;
                }

                $prop = mb_substr($key, 0, mb_strlen($key) - 3);
                $this->$prop;
            }
        }
        ...
        foreach ($this as $key => $value) {
            if (!$onlyMe && is_array($value)) {
                ...
                if ($hasFilters) {
                    if (!in_array(mb_strtolower($value->getMeta('type')), $filters)) {
                        continue;
                    }
                }
    ...
    public function __isset($property)
    {
        ...
        if (mb_strpos($property, 'xown') === 0 && ctype_upper(mb_substr($property, 4, 1))) {
    ...
    public function beau($property)
    {
        ...
        if (
            (mb_strpos($property, 'own') === 0 && ctype_upper(mb_substr($property, 3, 1)))
            || (mb_strpos($property, 'xown') === 0 && ctype_upper(mb_substr($property, 4, 1)))
            || (mb_strpos($property, 'shared') === 0 && ctype_upper(mb_substr($property, 6, 1)))
        ) {
    ...
    public function isListInExclusiveMode($listName)
    {
        ...
        if (mb_strpos($listName, 'xown') === 0 && ctype_upper(mb_substr($listName, 4, 1))) {
            $listName = mb_substr($listName, 1);
        }

        $listName = lcfirst(mb_substr($listName, 3));
    ...
    public function &__get($property)
    {
        ...
        if (!ctype_lower($property)) {
            ...
            if (mb_strpos($property, 'xown') === 0 && ctype_upper(mb_substr($property, 4, 1))) {
                $property = mb_substr($property, 1);
                $listName = lcfirst(mb_substr($property, 3));
                $isEx = true;
                $isOwn = true;
                $this->__info['sys.exclusive-'.$listName] = true;
            } elseif (mb_strpos($property, 'own') === 0 && ctype_upper(mb_substr($property, 3, 1))) {
                $isOwn = true;
                $listName = lcfirst(mb_substr($property, 3));
            } elseif (mb_strpos($property, 'shared') === 0 && ctype_upper(mb_substr($property, 6, 1))) {
                $isShared = true;
            }
        }
        ...
        if (isset($this->$fieldLink)) {
            ...
            if (isset($this->__info["sys.parentcache.$property"])) {
                ...
                $bean = null;
                if (null !== $this->properties[$fieldLink]) {
        ...
        if ($this->noLoad) {
            ...
            $beans = $this->getSharedList(lcfirst(mb_substr($property, 6)), $redbean, $toolbox);
        }
     ...
     * @param mixed  $value        value
     * @param bool   $updateShadow whether you want to update the shadow
     * @param bool   $taint        whether you want to mark the bean as tainted
     *
     * @return void
     */
    ...
    public function __call($method, $args)
    {
        ...
        if (!method_exists($this->__info['model'], $method)) {
            ...
            if (in_array($method, [ 'update', 'open', 'delete', 'after_delete', 'after_update', 'dispense' ], true)) {
                return null;
            }
     ...
     * @param int      $maxDepth maximum depth for traversal
     *
     * @throws RedException
     * @return OODBBean
     */
    ...
    {
        $this->via = null;
        if (mb_strpos($property, 'shared') !== false) {
    ...
    public function countOwn($type)
    {
        ...
        if ($this->getID()) {
            ...
            if (!is_numeric($firstKey) || $firstKey === null) {
                $bindings = $this->withParams;
                $bindings[':slot0'] = $this->getID();
                $count = $this->beanHelper->getToolbox()->getWriter()->queryRecordCount($type, [], " {$joinSql} $myFieldLink = :slot0 ".$this->withSql, $bindings);
            } else {
                $bindings = array_merge([ $this->getID() ], $this->withParams);
                $count = $this->beanHelper->getToolbox()->getWriter()->queryRecordCount($type, [], " {$joinSql} $myFieldLink = ? ".$this->withSql, $bindings);
            }
        }
    ...
    public function &aggr($list, $property, $type = null)
    {
        ...
        if (mb_strpos($list, 'own') !== 0) {
            throw new RedException('Only own-lists can be aggregated.');
        }
        if (!ctype_upper(mb_substr($list, 3, 1))) {
            throw new RedException('Invalid own-list.');
        }
...
interface Adapter
{
     ...
     * @param bool   $noevent  no event firing
     *
     * @return void
     */
...
class BeanCollection
{
    ...
    public function next()
    {
        ...
        if ($row) {
            $beans = $this->repository->convertToBeans($this->type, [ $row ]);
...
interface QueryWriter
{
     ...
     * @param int    $glue the GLUE type - how to glue (C_GLUE_WHERE or C_GLUE_AND)
     *
     * @return string
     */
     ...
     * @param int    $field  data type for field
     *
     * @return void
     */
     ...
     * @param bool   $includeSpecials whether you want to get codes for special columns as well
     *
     * @return int
     */
     ...
     * @param int    $datatype target data type
     *
     * @return void
     */
     ...
     * @return Cursor
     */
    public function queryRecordWithCursor($type, $addSql = null, $bindings = []);
     ...
     * @param bool   $all      if TRUE only return records that have been associated with ALL the tags in the list
     ...
     * @param int    $id           optional primary key ID value
     *
     * @return int
     */
     ...
     * @param bool   $noQuotes          TRUE to NOT put backticks or quotes around the string
     *
     * @return string
     */
...
abstract class AQueryWriter
{
     ...
     *
     * @param bool $narrowField TRUE = Narrow Field FALSE = Wide Field
     *
     * @return void
     */
    ...
    {
        $sql = $this->adapter->getSQL();
        if (mb_strpos($sql, '-- keep-cache') !== mb_strlen($sql) - 13) {
     ...
     * @param bool   $noQuote    TRUE if you want to omit quotes
     *
     * @return array
     */
     ...
     * @param int   $offset        start counter at...
     *
     * @return string
     */
     ...
     *
     * @param int    $dataTypeID    magic number constant assigned to this data type
    ...
    protected function startsWithZeros($value)
    {
        ...
        if (mb_strlen($value) > 1 && mb_strpos($value, '0') === 0 && mb_strpos($value, '0.') !== 0) {
            return true;
        } else {
            return false;
        }
    ...
    public function glueSQLCondition($sql, $glue = null)
    {
        ...
        if (preg_match('/^(INNER|LEFT|RIGHT|JOIN|AND|OR|WHERE|ORDER|GROUP|HAVING|LIMIT|OFFSET)\s+/i', $lsql)) {
            if ($glue === QueryWriter::C_GLUE_WHERE && mb_stripos($lsql, 'AND') === 0) {
                $snippetCache[$key] = ' WHERE '.mb_substr($lsql, 3);
            } else {
    ...
    public function glueLimitOne($sql = '')
    {
        return (mb_strpos(mb_strtoupper($sql), 'LIMIT') === false) ? ($sql.' LIMIT 1 ') : $sql;
    }
    ...
    public function queryTagged($type, $tagList, $all = false, $addSql = '', $bindings = [])
    {
        $assocType = $this->getAssocTable([ $type, 'tag' ]);
        ...
        $bindings = array_merge($tagList, [ $score ], $bindings);
        $rows = $this->adapter->get($sql, $bindings);
        return $rows;
    }
    ...
    {
        if (null !== $newMaxCacheSizePerType && $newMaxCacheSizePerType > 0) {
     ...
     * @param bool   $noQuotes omit quotes
     *
     * @return string
     */
...
class MySQL extends AQueryWriter implements QueryWriter
{
    ...
    protected function getKeyMapForType($type)
    {
		...
		', [ ':database' => $databaseName, ':table' => $table ]);
        $keyInfoList = [];
        foreach ($keys as $k) {
    ...
    public function __construct(Adapter $adapter)
    {
        ...
        foreach ($this->typeno_sqltype as $k => $v) {
            $this->sqltype_typeno[trim(mb_strtolower($v))] = $k;
        }
    ...
    public function addFK($type, $targetType, $property, $targetProperty, $isDependent = false)
    {
        ...
        $tableNoQ = $this->esc($type, true);
        $fieldNoQ = $this->esc($property, true);
        if (null !== $this->getForeignKeyForTypeProperty($tableNoQ, $fieldNoQ)) {
            return false;
        }
...
class SQLiteT extends AQueryWriter implements QueryWriter
{
    ...
    protected function putTable($tableMap)
    {
        ...
        foreach ($tableMap['indexes'] as $name => $index) {
            if (mb_strpos($name, 'UQ_') === 0) {
                $cols = explode('__', mb_substr($name, mb_strlen('UQ_'.$table)));
                foreach ($cols as $k => $v) {
                    $cols[$k] = "`$v`";
                }
     ...
     * @param int    $buildopt    0 = NO ACTION, 1 = ON DELETE CASCADE
     *
     * @return bool
     */
    protected function buildFK($type, $targetType, $property, $targetProperty, $constraint = false)
    {
        ...
        if (null !== $this->getForeignKeyForTypeProperty($table, $column)) {
            return false;
        }
     ...
     * foreign keys. Returns one if the data type constants.
     *
     * @return int $const data type to be used for IDS.
     */
    public function getTypeForID()
    {
...
abstract class Repository
{
    ...
    protected function storeBeanWithLists(OODBBean $bean)
    {
        ...
        foreach ($bean as $property => $value) {
            ...
            if ($value instanceof OODBBean) {
                ...
                $originals = $bean->moveMeta('sys.shadow.'.$property, []);
                if (mb_strpos($property, 'own') === 0) {
                    ...
                    unset($bean->$property);
                } elseif (mb_strpos($property, 'shared') === 0) {
     ...
     */
    public function findCollection($type, $sql, $bindings = [])
    {
        try {
...
class Fluid extends Repository
{
    ...
    private function modifySchema(OODBBean $bean, $property, $value)
    {
        ...
        if (!$this->oodb->isChilled($bean->getMeta('type'))) {
            ...
            if ($doFKStuff) {
                if (mb_strrpos($columnNoQ, '_id') === (mb_strlen($columnNoQ) - 3)) {
                    $destinationColumnNoQ = mb_substr($columnNoQ, 0, mb_strlen($columnNoQ) - 3);
...
class OODB extends Observable
{
     ...
     *
     * @param bool $autoClear auto clear option
     *
     * @return void
     */
     ...
     * @param array|bool  $frozen mode of operation: TRUE (frozen), FALSE (default, fluid) or ARRAY (chilled)
     */
     ...
     *
     * @param bool|array $toggle TRUE if you want to use OODB instance in frozen mode
     *
     * @return void
     */
     ...
     * @return array
     */
    public function findCollection($type, $sql = null, $bindings = [])
    {
    ...
    public function bindFunc($mode, $field, $function)
    {
        ...
        if (!isset(self::$sqlFilters[$mode])) {
            self::$sqlFilters[$mode] = [];
        }
        if (!isset(self::$sqlFilters[$mode][$type])) {
            self::$sqlFilters[$mode][$type] = [];
        }
...
class Finder
{
     ...
     * @return OODBBean
     */
    public function findOrCreate($type, $like = [])
    {
     ...
     * @return array
     */
    public function findLike($type, $conditions = [], $sql = '')
    {
     ...
     * @return array
     */
    public function findMulti($types, $sql, $bindings = [], $remappings = [], $queryTemplate = ' %s.%s AS %s__%s')
    {
        if (!is_array($types)) {
            $types = explode(',', $types);
        }
        if (!is_array($sql)) {
            ...
            foreach ($types as $type) {
                $pattern = " {$type}.*";
                if (mb_strpos($sql, $pattern) !== false) {
                    $newSelectorArray = [];
        ...
        foreach ($types as $type) {
            ...
            foreach ($rows as $rowkey => $row) {
                ...
                foreach ($row as $cell => $value) {
                    if (mb_strpos($cell, $prefix) === 0) {
                        $property = mb_substr($cell, mb_strlen($prefix));
...
class AssociationManager extends Observable
{
    ...
    protected function associateBeans(OODBBean $bean1, OODBBean $bean2, OODBBean $bean)
    {
        ...
        $bean->setMeta('sys.buildcommand.unique', [ $property1, $property2 ]);
     ...
     * @param bool     $fast  if TRUE, removes the entries by query without FUSE
     *
     * @return void
     */
...
class SimpleModelHelper implements Observer
{
    ...
    {
        foreach ([ 'update', 'open', 'delete', 'after_delete', 'after_update', 'dispense' ] as $eventID) {
...
class TagManager
{
     ...
     * @param bool         $all  whether they must all match or just some
     *
     * @return bool
     */
...
class LabelMaker
{
    ...
    public function enum($enum)
    {
        ...
        if (mb_strpos($enum, ':') === false) {
            $type = $enum;
            $value = false;
        } else {
            list($type, $value) = explode(':', $enum);
            $value = preg_replace('/\W+/', '_', mb_strtoupper(trim($value)));
        }
...
class Facade
{
     ...
     * @param bool   $frozen   TRUE if you want to setup in frozen mode
     *
     * @return ToolBox
     */
     ...
     *
     * @param bool $mode TRUE = Narrow Field Mode
     *
     * @return void
     */
    ...
    public static function addDatabase($key, $dsn, $user = null, $pass = null, $frozen = false)
    {
        ...
        if (is_object($dsn)) {
            ...
            $dbType = mb_substr($dsn, 0, mb_strpos($dsn, ':'));
        }
        ...
        $wkey = trim(mb_strtolower($dbType));
     ...
     * debug messages.
     *
     * @param bool $tf   debug mode (TRUE or FALSE)
     ...
     * in one call.
     *
     * @param bool $toggle TRUE to activate debugger and select 'fancy' mode
     *
     * @return void
     */
     ...
     * @param int          $number            number of beans to dispense
     * @param bool         $alwaysReturnArray if TRUE always returns the result as an array
     *
     * @return array|OODBBean
     */
     ...
     * @param bool   $onlyArrays return only arrays even if amount < 2
     *
     * @return array
     */
     ...
     * @return OODBBean
     */
    public static function findOneOrDispense($type, $sql = null, $bindings = [])
    {
     ...
     * @return BeanCollection
     */
    public static function findCollection($type, $sql = null, $bindings = [])
    {
     ...
     * @return array
     */
    public static function findMulti($types, $sql, $bindings = [], $remappings = [])
    {
     ...
     * @param bool     $pid   for internal usage
     ...
     * @return array
     */
    public static function duplicate($bean, $filters = [])
    {
        return self::dup($bean, [], false, $filters);
    }
     ...
     * @param bool           $parents whether you want parent beans to be exported
    ...
    {
        if (!in_array($caseStyle, [ 'default', 'camel', 'dolphin' ])) {
    ...
    {
        $beans = self::$redbean->convertToBeans($type, [ $row ], $metamask);
        $bean = reset($beans);
        return $bean;
    }
     ...
     *
     * @param bool $yesNo TRUE to enable cache, FALSE to disable cache
     *
     * @return void
     */
    ...
    {
        return [ self::$redbean, self::$adapter, self::$writer, self::$toolbox ];
    }
     ...
     * This method returns the old mode and handler as an array.
     *
     * @param int           $mode mode, determines how to handle errors
     ...
     * @return OODBBean
     */
    public static function findOrCreate($type, $like = [])
    {
     ...
     * @return array
     */
    public static function findLike($type, $like = [], $sql = '')
    {
...
class DuplicationManager
{
     ...
     * @param bool     $preserveIDs TRUE means preserve IDs, for export only
     *
     * @return void
     */
     ...
     * @param bool     $preserveIDs preserve IDs
     *
     * @return OODBBean
     */
     ...
     * @param bool  $dolphinMode whether you want the exception for IDs.
     *
     * @return array
     */
     ...
     *
     * @param bool $yesNo TRUE to use caching, FALSE otherwise
     */
     ...
     * @param OODBBean $bean        bean to be copied
     * @param array    $trail       for internal usage, pass array()
     * @param bool     $preserveIDs for internal usage
     *
     * @return OODBBean
     */
     ...
     *
     * @param array|OODBBean $beans     beans to be exported
     * @param bool           $parents   also export parents
...
class ArrayTool
{
    ...
    public static function genSlots($array, $template = null)
    {
        ...
        return (null === $template || $str === '') ? $str : sprintf($template, $str);
    }
